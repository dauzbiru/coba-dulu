...
@section('title', 'Edit Checklist - Monapps')

@section('content')
    <div class="max-w-lg mx-auto bg-white rounded-xl shadow-md p-6 sm:p-8">
        <h1 class="text-xl sm:text-2xl font-bold text-gray-800 mb-6">Edit Checklist</h1>
        ...
        <form method="POST" action="/items/{{ $item->id }}">
            ...
            <div class="mb-6">
                ...
                <input id="bobot" type="number" step="0.01" name="bobot" value="{{ old('bobot', $item->bobot) }}"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                @error('bobot')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="flex gap-3">
                <a href="/categories/{{ $item->category_id }}" class="flex-1 text-center px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 text-sm font-medium">Batal</a>
