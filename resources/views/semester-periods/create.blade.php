...
@section('title', 'Tambah Periode Semester - Monapps')

@section('content')
    <div class="max-w-lg mx-auto bg-white rounded-xl shadow-md p-6 sm:p-8">
        <h1 class="text-xl sm:text-2xl font-bold text-gray-800 mb-6">Tambah Periode Semester</h1>
        ...
        <form method="POST" action="/semester-periods">
            ...
            <div class="mb-4">
                ...
                <input id="year" type="number" name="year" min="2000" max="2099" value="{{ old('year') }}" required
            ...
            <div class="grid grid-cols-2 gap-4 mb-6">
                <div>
                    ...
                    <input id="start_month" type="number" name="start_month" min="1" max="12" value="{{ old('start_month') }}" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    @error('start_month')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input id="end_month" type="number" name="end_month" min="1" max="12" value="{{ old('end_month') }}" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    @error('end_month')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
